...
- consistency between predicted label for corresponding noun out of context and predicted label in transitive
- correlation between LL_delta in the transitive and the correspinding noun
"""
...
def calculate_noun_accuracy_and_overlap(df, dir, prompt_type):
    df_noun_acc = pd.DataFrame(columns=['noun', 'num_counts', 'accuracy', 'avg_LL_delta'])
    ...
    for noun in nouns:
        ...
        row = [noun, noun_total, num_correct/noun_total, avg_LL_delta]
        df_noun_acc.loc[len(df_noun_acc.index)] = row
    df_noun_acc = df_noun_acc.sort_values(['noun'])
    df_noun_acc.to_csv(dir + '/noun_accuracy_' + prompt_type + '.csv', index=False)
    return

def calculate_per_verb_accuracy(df, dir, prompt_type):
    df_verb_acc = pd.DataFrame(columns=['verb', 'overall_acc', 'avg_LL_delta'])
    ...
    for verb in verbs:
        ...
        avg_LL_delta = df_verb['LL_delta'].mean()
        row = [verb, num_correct/verb_total, avg_LL_delta]
        df_verb_acc.loc[len(df_verb_acc.index)] = row
    df_verb_acc = df_verb_acc.sort_values(['verb'])
    df_verb_acc.to_csv(dir + '/verb_accuracy_' + prompt_type + '.csv', index=False)
    return  
...
def results(prompt_type, folder, exp, exp_results_dir):
    df_corr = pd.DataFrame(columns=['model', 'corr_noun_LL', 'corr_human', 'corr_subj'])
    df_acc = pd.DataFrame(columns=['model', 'overall_acc'])
    for filepath in folder:
        format_scores(filepath)
        ...
        calculate_noun_accuracy_and_overlap(df, model_results_dir, prompt_type)
        calculate_per_verb_accuracy(df, model_results_dir, prompt_type)
    ...
    df_corr.to_csv(exp_results_dir + '/' + prompt_type + '_corr.csv')
    ...
    df_acc.to_csv(exp_results_dir + '/' + prompt_type + '_acc.csv')
    return
...
def main():
    ...
    for exp in exp_folders:
        ...
        trans_subj_folder = exp + '/trans_subj_scores/*'
        trans_subj_score_files = glob(trans_subj_folder)
        results("trans_subj", trans_subj_score_files, exp, exp_results_dir)
        trans_obj_folder = exp + '/trans_obj_scores/*'
        trans_obj_score_files = glob(trans_obj_folder)
        results("trans_obj", trans_obj_score_files, exp, exp_results_dir)
