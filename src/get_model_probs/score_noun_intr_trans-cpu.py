...
def get_args():
    ...
    parser.add_argument("--cache_dir", type=str, default="/projects/ogma3/ltjuatja/huggingface/models/")
    args = parser.parse_args()
    print(f"args: {vars(args)}")
    return args

def get_model_and_tokenizer(model_name, model_size, cache_dir):
    cache_path = cache_dir
    name_and_size = model_name + '-' + model_size
    if model_size == 'x':
        name_and_size = model_name
    if model_name == 'gpt2':
        model = GPT2LMHeadModel.from_pretrained(name_and_size, cache_dir=cache_path)
        ...
        model = BloomForCausalLM.from_pretrained("bigscience/" + name_and_size, cache_dir=cache_path)
    else:   
...
def main():
    ...
    model,tokenizer = get_model_and_tokenizer(model_name, model_size, cache_dir)
